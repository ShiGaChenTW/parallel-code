...
- 十個同時可見任務時，狀態與 terminal output 更新不應阻塞 UI 操作。
...
| Q3  | 要不要正式承諾完全離線模式                      | **要**。需總開關涵蓋全部對外連線點                     |
...
2. **Q1 與第 7.4 節的效能承諾不一致。** 該節只承諾「十個同時可見任務時不阻塞 UI」，
   但 Q1 已裁決不設上限，等於這個數字沒有對應的驗證基準。需決定是提高承諾並補驗證，
   還是改寫為「已驗證至十個」。此項未裁決前，第 7.4 節照原文有效。
