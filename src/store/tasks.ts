...
export interface CreateTaskOptions {
  ...
  /** The one task this one depends on. When set and git isolation is
   *  `worktree`, the new worktree forks from that task's branch instead of
   *  `baseBranch` — the git-ancestry half of a dependency edge, done entirely
   *  by substituting a string that `createWorktree` already accepts. */
...
export async function createTask(opts: CreateTaskOptions): Promise<string> {
  ...
  // Resolve the dependency before anything is created, so a stale pick fails
  // with a sentence instead of producing a worktree that is blocked on arrival.
  const dependsOnTaskId = opts.dependsOnTaskId;
  const dependency = dependsOnTaskId ? store.tasks[dependsOnTaskId] : undefined;
  if (dependsOnTaskId && !dependency) {
    throw new Error('The task this one depends on no longer exists');
  }
  // Only worktree isolation forks a branch. A 'direct' task works *on*
  // baseBranch, and the dependency's branch is checked out in the dependency's
  // own worktree — git would refuse it. The scheduling half of the edge still
  // applies in every mode.
  const effectiveBaseBranch =
    gitIsolation === 'worktree' ? resolveDependencyBaseBranch(dependency, baseBranch) : baseBranch;
...
/**
 * Drop a task's dependency edge, letting its agent start.
 *
 * The deliberate counterpart to `closeTask` NOT detaching dependents. Closing a
 * dependency leaves the dependent blocked with a reason (決議 3) rather than
 * silently rebasing its expectations onto the project base branch; this is the
 * one explicit action that clears that block, and it is the user's to take.
 *
 * The branch the worktree already forked from is left alone — it is a fact
 * about git history, not a preference, and rewriting the field would not move
 * the commits.
 */
export function clearTaskDependency(taskId: string): void {
  if (!store.tasks[taskId]?.dependsOnTaskId) return;
  setStore('tasks', taskId, 'dependsOnTaskId', undefined);
  saveState();
}
