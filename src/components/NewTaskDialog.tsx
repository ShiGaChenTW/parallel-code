...
import { HulyIssuePicker } from './HulyIssuePicker';
import { taskNameForIssue } from '../lib/huly-issues';
...
export function NewTaskDialog(props: NewTaskDialogProps) {
  ...
  const [hulyIssue, setHulyIssue] = createSignal<HulyIssue | null>(null);
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      ...
      const taskId = await createTask({
        ...
        hulyIssueId: hulyIssue()?.id,
        hulyIssueIdentifier: hulyIssue()?.identifier,
        agentDef: agent,
        projectId,
        ...
        >
          ...
          >
            <HulyIssuePicker
              onSelect={(issue) => {
                setHulyIssue(issue);
                setName(taskNameForIssue(issue));
              }}
            />
            <label style={sectionLabelStyle}>
              Task name{' '}
