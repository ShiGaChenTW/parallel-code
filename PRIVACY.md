...
- **Huly** — only after you enter a Huly server URL, workspace and access token in Settings. Parallel Code then connects to that server over a WebSocket to **read** issues for the project you name, so an issue can be turned into a task. It is read-only: nothing is written back to Huly. What leaves your machine is the connection itself (your IP, and the token as your credential); what comes back is issue data from your own workspace. The server is one you or your organisation operates — this project neither runs nor receives anything from it. Your token is encrypted with your operating system's keychain via Electron's `safeStorage` and written to `huly-credentials.enc` in the app's data directory; if the OS reports that encryption is unavailable (a Linux machine with no keyring running, for example), the app refuses to store the token rather than writing it in the clear. Clearing the credentials in Settings deletes that file and closes the connection. Plain `http://` URLs are rejected for anything but loopback, so the token is not sent unencrypted over a network.
